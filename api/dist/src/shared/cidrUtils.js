...
export function checkOverlap(cidr1, cidr2) {
    try {
        ...
        if (isIPv6_1) {
            ...
            if (addr1.isInSubnet(addr2))
                return 'subset';
            if (addr2.isInSubnet(addr1))
                return 'superset';
...
export function findConflicts(ranges) {
    const conflicts = [];
...
function getConflictDescription(range1, range2, type) {
    switch (type) {
        case 'subset':
...
function getImpactDescription(type, isSameSubscription) {
...
//# sourceMappingURL=cidrUtils.js.map